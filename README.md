...
Полный бэкап — один `.tar.gz`, внутри:
- `database.dump` — дамп Postgres (`pg_dump -Fc`),
- `objects/` — все файлы object storage (фото паспортов и т.п., PII),
- `manifest.json` — id бэкапа, дата, версия схемы Alembic, имя БД, число файлов.

Управление — в админ-панели на странице **Бэкап** (только platform-admin):
расписание (выкл / ежедневно в заданное время Asia/Tashkent / по интервалу),
ретеншн (сколько последних копий хранить), доставка в Telegram
(целиком / только БД / разбить на части), кнопка «Сделать бэкап сейчас»,
история запусков со скачиванием и отправкой в Telegram, и загрузка архива
для восстановления.

**CLI** (из `backend/`):
```bash
uv run python -m scripts.backup                          # создать бэкап
uv run python -m scripts.restore <archive.tar.gz> --yes  # восстановить (затрёт БД и бакет!)
#   добавьте --force, чтобы пропустить проверку совпадения версии схемы
```

Архивы пишутся в `BACKUP_DIR` (по умолчанию `/backups`, том `backup_data`).
`pg_dump`/`pg_restore` 16 встроены в backend-образ.

> ⚠️ **Restore необратим**: `pg_restore --clean` сбрасывает и пересоздаёт
> таблицы, файлы заливаются поверх. Возможен короткий простой. По умолчанию
> restore отклоняет архив с другой версией схемы Alembic (обойти — `--force`
> или галка в админке).

**Сборка частей из Telegram** (если бэкап отправлен по частям):
```bash
cat malika-backup-<id>-<дата>.part* > archive.tar.gz
```
