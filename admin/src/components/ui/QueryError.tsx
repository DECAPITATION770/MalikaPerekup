...
export default function QueryError({ onRetry, error, inline }: Props) {
  ...
  return (
    <div role="alert" className="flex flex-col items-center gap-3 px-6 py-12 fia">
      <div className="flex h-12 w-12 items-center justify-center rounded-card border border-danger/30 bg-danger-faded">
        <Icon size={20} className="text-danger" />
      </div>
      <div className="max-w-xs text-center text-label text-text-dim">{message}</div>
      {onRetry && (
